...
class triad_openvr():
    def __init__(self, configfile_path=None):
        ...
        for i in range(3):
            try:
                ...
                print("Failed to connect to VR system, trying again %s" % (i+1))
                time.sleep(2)
        ...
        for i in range(openvr.k_unMaxTrackedDeviceCount):
            if poses[i].bDeviceIsConnected:
                self.add_tracked_device(i, verbose=False)
    ...
    def add_tracked_device(self, tracked_device_index, verbose=True):
        ...
        if not known_device_found:
            ...
            if (device_class == openvr.TrackedDeviceClass_Controller):
                device_name = "controller_" + device_serial
    ...
    def print_device_info(self, device_name):
        device_type = self.devices[device_name].device_class
